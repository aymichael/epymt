...
export default function QuestionBox({ question, answer }) {
    ...
    return (
        <div className="border-2 border-black p-2 px-4 rounded-2xl flex flex-col gap-2">
            <div className={`font-sans font-bold cursor-pointer select-none md:text-lg sm:text-base flex justify-between items-center gap-4 text-sm ${showAnswer ?  "text-black" : "text-[#993300]"} `} onClick={() => setShowAnswer(!showAnswer)}>
                {question}
