  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <h3 className="text-xl font-semibold mb-2 text-gray-800 dark:text-gray-200">{lang === 'en' ? 'Outstanding Awards' : '卓越表現獎'}</h3>
          {outstandingNameList.map((name, index) => (
            <AwardCard key={index} name={name} school={outstandingSchoolList[index]} />
          ))}
        </div>
        <div>
          <h3 className="text-xl font-semibold mb-2 text-gray-800 dark:text-gray-200">{lang === 'en' ? 'Excellent Awards' : '優異表現獎'}</h3>
